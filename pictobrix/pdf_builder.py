...
def _overview_page(c, m: Mosaic, drawers, sym_order, total_pages, name):
    ...
    # resolucion dinamica: mosaicos mas grandes -> mas pixeles por celda,
    # apuntando a ~2048 px en el lado largo (acotado para no inflar el PDF)
    target_px = 2048
    block = max(8, min(32, round(target_px / max(m.width_px, m.height_px))))
    preview = render_preview(m, block=block)
